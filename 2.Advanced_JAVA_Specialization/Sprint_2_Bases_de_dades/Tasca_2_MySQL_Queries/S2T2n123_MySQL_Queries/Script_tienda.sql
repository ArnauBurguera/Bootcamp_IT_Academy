...
-- 4Llista el nom dels productes, el preu en euros i el preu en dòlars estatunidencs (USD).
	-- Asumeixo que el preu actual està en euros. Conversió 1 € = 1.09$.
SELECT nombre, precio as "precio €", ROUND(precio * 1.09,2) as "precio $" FROM producto;

-- 5 Llista el nom dels productes, el preu en euros i el preu en dòlars estatunidencs (USD). Utilitza els següents àlies per a les columnes: nom de producto, euros, dòlars.
SELECT nombre, precio as "euros", ROUND(precio * 1.09,2) as "dòlars" FROM producto;
...
SELECT nombre, UPPER(LEFT(nombre, 2)) as "abreviación" FROM fabricante;
...
SELECT p.nombre, p.precio, f.nombre as "nom fabricant" FROM producto p JOIN fabricante f ON f.codigo = p.codigo_fabricante;

-- 22 Retorna una llista amb el nom del producte, preu i nom de fabricant de tots els productes de la base de dades. Ordena el resultat pel nom del fabricant, per ordre alfabètic.
SELECT p.nombre, p.precio, f.nombre as "nom fabricant" FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo ORDER BY f.nombre ASC;
...
SELECT p.nombre, p.precio, f.nombre as "nom fabricant" FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo ORDER BY p.precio ASC LIMIT 1;

-- 25 Retorna el nom del producte, el seu preu i el nom del seu fabricant, del producte més car.
SELECT p.nombre, p.precio, f.nombre as "nom fabricant" FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo ORDER BY p.precio DESC LIMIT 1;
...
SELECT * FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE precio > 200 and f.nombre = "Crucial";
...
-- 30 Retora un llistat amb el nom i el preu de tots els productes dels fabricants el nom dels quals acabi per la vocal e.
SELECT p.nombre, p.precio FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE f.nombre LIKE '%e';

-- 31 Retorona un llistat amb el nom i el preu de tots els productes el nom de fabricant dels quals contingui el caràcter w en el seu nom.
...
SELECT p.nombre, p.precio, f.nombre FROM producto p JOIN fabricante f ON p.codigo_fabricante = f.codigo WHERE p.precio >= 180 ORDER BY p.nombre ASC;
